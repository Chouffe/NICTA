...
spaces = P $ \input ->
  case input of
    (' ':.input') ->
      case parse spaces input' of
        Result input'' xs -> Result input'' (' ':.xs)
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok cstart cend = between (charTok cstart) (charTok cend)
