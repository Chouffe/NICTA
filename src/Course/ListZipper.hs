...
instance Functor ListZipper where
  f <$> ListZipper l focus r = ListZipper (f <$> l) (f focus) (f <$> r)
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l focus r) = reverse l ++ (focus:.Nil) ++ r
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper l focus r) = ListZipper l (f focus) r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l focus r) =
  case break p l of
    (_, Nil)     -> IsNotZ
    (Nil, z:.zs) -> IsZ $ ListZipper zs z (focus :. r)
    (as, b:.bs)  -> IsZ $ ListZipper bs b (as ++ (focus :. r))
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l focus r) =
  case break p r of
    (_, Nil)     -> IsNotZ
    (Nil, z:.zs) -> IsZ $ ListZipper (focus :. l) z zs
    (as, b:.bs)  -> IsZ $ ListZipper (reverse as ++ (focus :. l)) b bs
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil focus r) = ListZipper l' focus' Nil
  where (focus':.ts) = reverse r
        l' = ts ++ (focus :. Nil)
moveLeftLoop (ListZipper (l:.ls) focus r) = ListZipper ls l (focus :. r)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l focus Nil) = ListZipper Nil focus' r'
  where (focus' :. ts) = reverse l
        r' = ts ++ (focus :. Nil)
moveRightLoop (ListZipper l focus (r:.rs)) = ListZipper (focus :. l) r rs
...
moveLeft (ListZipper (l:.ls) focus r) = IsZ (ListZipper ls l (focus :. r))
...
moveRight (ListZipper l focus (r:.rs)) = IsZ (ListZipper (focus :. l) r rs)
...
swapLeft (ListZipper (l :. ls) focus r) = IsZ (ListZipper (focus :. ls) l r)
...
swapRight (ListZipper l focus (r:.rs)) = IsZ (ListZipper l r (focus :. rs))
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ focus r) = ListZipper Nil focus r
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper l focus _) = ListZipper l focus Nil
...
moveLeftN' n lz
  | n < 0 = case moveRight lz of
              IsNotZ  -> Left 0
              IsZ lz' -> case moveLeftN' (n + 1) lz' of
                           Left m     -> Left (m + 1)
                           Right lz'' -> Right lz''
  | n > 0 = case moveLeft lz of
              IsNotZ  -> Left 0
...
moveRightN' n lz
  | n < 0 = case moveLeft lz of
              IsNotZ  -> Left 0
              IsZ lz' -> case moveRightN' (n + 1) lz' of
                           Left m     -> Left (m + 1)
                           Right lz'' -> Right lz''
  | n > 0 = case moveRight lz of
              IsNotZ  -> Left 0
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper l focus r) = ListZipper l x (focus:.r)
...
instance Applicative ListZipper where
  ...
  (ListZipper fl ffocus fr) <*> (ListZipper l focus r) =
    ListZipper (zipWith ($) fl l) (ffocus focus) (zipWith ($) fr r)
...
instance Extend ListZipper where
  f <<= lz = ListZipper l (f lz) r
    where l = unfoldr (\lz' -> case moveLeft lz' of
                                 IsNotZ   -> Empty
                                 IsZ lz'' -> Full (f lz'', lz'')) lz
          r = unfoldr (\lz' -> case moveRight lz' of
                                 IsNotZ   -> Empty
                                 IsZ lz'' -> Full (f lz'', lz'')) lz
...
instance Traversable ListZipper where
  traverse f (ListZipper l focus r) = pure ListZipper <*> traverse f l <*> f focus <*> traverse f r
