...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f st = State $ \s -> let (a, s1) = runState st s
                             in (f a, s1)
...
findM p (x:.xs) =
  (\bool -> if bool then pure (Full x) else findM p xs) =<< p x
...
firstRepeat ::
  ...
  -> Optional a
firstRepeat xs =
  eval (findM (\x -> State $ \set -> (S.member x set, S.insert x set)) xs) S.empty
...
distinct ::
  ...
  -> List a
distinct xs =
  eval (filtering (\x -> State $ \set -> (not $ S.member x set, S.insert x set)) xs) S.empty
