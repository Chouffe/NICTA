...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f (StateT st) = StateT $ \s -> (\(x, s1) -> (f x, s1)) <$> st s
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  fasmb =<< sma = StateT $ \s -> ((\(a, s1) -> runStateT (fasmb a) s1) =<< runStateT sma s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' xs = eval' (filtering (\x -> state' (\set -> (not $ S.member x set, S.insert x set))) xs) S.empty
...
distinctF ::
  ...
  -> Optional (List a)
distinctF xs =
  evalT (filtering (\x -> StateT $ \set -> if x > 100 then Empty else Full (not $ S.member x set, S.insert x set)) xs) S.empty
...
instance Applicative (Logger l) where
  pure = Logger Nil
  (Logger v fa) <*> (Logger w a) = Logger (v ++ w) (fa a)
...
instance Monad (Logger l) where
  f =<< (Logger w a) = let (Logger v b) = f a
                       in Logger (w ++ v) b
